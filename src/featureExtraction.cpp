...
public:
    ...
    void markOccludedPoints()
    {
        ...
        for (int i = 5; i < cloudSize - 6; ++i)
        {
            ...
            // 两个激光点之间的水平方向索引差值，如果在同一条扫描线上，那么值为1；
            ...
            // 2.parallel beam
            // 用前后相邻点判断当前点所在平面是否与激光束方向平行(入射角太大)
            // diff1和diff2是当前点距离前后两个点的距离
            float diff1 = std::abs(float(cloudInfo.pointRange[i - 1] - cloudInfo.pointRange[i]));
            float diff2 = std::abs(float(cloudInfo.pointRange[i + 1] - cloudInfo.pointRange[i]));
            // 如果当前点距离左右邻点都过远，则视其为瑕点，因为入射角可能太大导致误差较大
            // 选择距离变化较大的点，并将他们标记为1
        ...
        for (int i = 0; i < N_SCAN; i++)
        {
            ...
            for (int j = 0; j < 6; j++)
            {
                ...
                /*
                   ...
                   假设当前ring在一维数组中起始点是m，结尾点为n（不包括n），
                   那么6段的起始点分别为：m + [(n-m)/6]*j   j从0～5, 化简为 [（6-j)*m + nj ]/6
                   6段的终止点分别为：m + (n-m)/6 + [(n-m)/6]*j -1  j从0～5,-1是因为最后一个,减去1, 化简为 [（5-j)*m + (j+1)*n ]/6 -1
