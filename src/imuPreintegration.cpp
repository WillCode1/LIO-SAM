...
public:
     ...
     * 2、发布当前时刻手动计算的imu里程计位姿，用于rviz展示；发布imu里程计路径，注：只是最近一帧激光里程计时刻与当前时刻之间的一段
     */
    void imuOdometryHandler(const nav_msgs::Odometry::ConstPtr &odomMsg)
    {
        ...
        // 当前时刻imu里程计位姿 = 最近的一帧激光里程计位姿 * (最近的一帧激光之后的)imu里程计增量位姿变换
        Eigen::Affine3f imuOdomAffineLast = lidarOdomAffine * imuOdomAffineIncre;
        float x, y, z, roll, pitch, yaw;
        pcl::getTranslationAndEulerAngles(imuOdomAffineLast, x, y, z, roll, pitch, yaw);

        // publish latest odometry
        // 发布当前时刻里程计位姿.名称为"odometry/imu"
        // 也就是说，这个函数先监听了类IMUPreintegration中发布的odometry/imu_incremental，
        // 然后计算上一个激光帧后imu里程计的累计增量，然后在激光的基础上加上增量，重新发布
...
public:
    ...
    IMUPreintegration()
    {
        ...
        // 设置IMU的参数：提前标定的重力加速度，加速度计协方差，陀螺仪协方差，积分协方差，零偏
    ...
    /**
     * 订阅激光里程计，来自mapOptimization, "lio_sam/mapping/odometry_incremental"
     * 1、每隔100帧激光里程计，重置ISAM2优化器，添加里程计、速度、偏置先验因子，执行优化
     * 2、测量模型：计算前一帧激光里程计与当前帧激光里程计之间的imu预积分量
     * 3、真值模型：用前一帧激光状态和来自mapOptimization优化的当前帧位姿，计算两项残差和雅各比并优化
     * 4、优化之后，执行重传播；优化更新了imu的偏置，用最新的偏置重新计算当前激光里程计时刻之后的imu预积分，这个预积分用于计算每时刻位姿
     */
    void odometryHandler(const nav_msgs::Odometry::ConstPtr &odomMsg)
    {
        ...
        if (systemInitialized == false)
        {
            ...
            // 图和节点均清零, 先验信息通过滑动窗口的原理保存在optimizer中
        ...
        // 添加imu因子，参数：前一帧位姿，前一帧速度，当前帧位姿，当前帧速度，前一帧偏置，imu预积分器
        ...
        if (!imuQueImu.empty())
        {
            ...
            // 计算预积分, 利用imuQueImu中的数据进行预积分 主要区别旧在于上一行的更新了bias
            for (int i = 0; i < (int)imuQueImu.size(); ++i)
            {
                sensor_msgs::Imu *thisImu = &imuQueImu[i];
                double imuTime = ROS_TIME(thisImu);
                double dt = (lastImuQT < 0) ? (1.0 / 500.0) : (imuTime - lastImuQT);
                // 注意:加入的是这个用于传播的的预积分器imuIntegratorImu_
    ...
    void imuHandler(const sensor_msgs::Imu::ConstPtr &imu_raw)
    {
        ...
        // 这里需要先在odometryHandler中优化一次后再进行该函数后续的工作
