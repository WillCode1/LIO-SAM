...
public:
    ...
    mapOptimization()
    {
        ...
        string saveKeyframeDirectory = std::getenv("HOME") + savePCDDirectory + "/keyFrameCloud/";
        if (!fs::exists(saveMapDirectory))
        {
            fs::create_directory(saveMapDirectory);
        }
        else
        {
            fs::remove_all(saveMapDirectory);
            fs::create_directory(saveMapDirectory);
        }
        if (saveKeyFramesPCD)
            fs::create_directory(saveKeyframeDirectory);
    ...
    void updateInitialGuess()
    {
        ...
        */
        ...
        // 做为odomAvailable = false时的备选方案, 即没有imu里程计数据时, 才使用imu角度数据更新
    ...
    void extractNearby()
    {
        ...
        // 把10s内的关键帧也加到surroundingKeyPosesDS中，以防机器人在一个位置原地旋转
    ...
    /**
     * 对当前帧点云降采样, 大量的降采样工作无非是为了使点云稀疏化 加快匹配以及实时性要求
     */
    ...
    void scan2MapOptimization()
    {
        ...
        if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum && laserCloudSurfLastDSNum > surfFeatureMinValidNum)
        {
            ...
            // 使用9轴imu的orientation与scan-to-map优化后的位姿进行加权融合，更新当前帧位姿的roll、pitch，约束z坐标
            transformUpdate();
        }
        else
        {
...
#pragma omp parallel for num_threads(numberOfCores)
        for (int i = 0; i < laserCloudCornerLastDSNum; i++)
        {
            ...
            if (pointSearchSqDis[4] < 1.0)
            {
                ...
                // 特征值分解, 求正交阵的特征值和特征向量
                // 特征值：matD1，特征向量：matV1, 对应于LOAM论文里雷达建图 特征值与特征向量那块
                cv::eigen(matA1, matD1, matV1);
                // 边缘：与较大特征值相对应的特征向量代表边缘线的方向（一大两小，大方向）, 原理类似于PCA
                ...
                if (matD1.at<float>(0, 0) > 3 * matD1.at<float>(0, 1))
                {
                    ...
                    // [la,lb,lc]=[la',lb',lc']/a012/l12, 得到底边上的高的单位向量[la,lb,lc]
                    ...
                    float la = ((y1 - y2) * ((x0 - x1) * (y0 - y2) - (x0 - x2) * (y0 - y1)) + (z1 - z2) * ((x0 - x1) * (z0 - z2) - (x0 - x2) * (z0 - z1))) / ld2;

                    float lb = -((x1 - x2) * ((x0 - x1) * (y0 - y2) - (x0 - x2) * (y0 - y1)) - (z1 - z2) * ((y0 - y1) * (z0 - z2) - (y0 - y2) * (z0 - z1))) / ld2;

                    float lc = -((x1 - x2) * ((x0 - x1) * (z0 - z2) - (x0 - x2) * (z0 - z1)) + (y1 - y2) * ((y0 - y1) * (z0 - z2) - (y0 - y2) * (z0 - z1))) / ld2;

                    // 下面涉及到一个鲁棒核函数，作者简单地设计了这个核函数。
                    // 距离越大，s越小，是个距离惩罚因子（权重），相当于协方差矩阵的逆
                    float s = 1 - 0.9 * fabs(ld2);

                    // 点到直线的垂线段单位向量(即直线方程系数, 用于表示偏导数)
                    coeff.x = s * la;
                    coeff.y = s * lb;
                    coeff.z = s * lc;
                    // 点到直线距离
                    ...
                    // 根据s的值来判断是否是边缘点
                    // s>0.1 也就是要求点到直线的距离ld2要小于1m, s越大说明ld2越小(离边缘线越近)
                    if (s > 0.1)
                    {
                        // 当前激光帧角点，加入匹配集合中
...
#pragma omp parallel for num_threads(numberOfCores)
        ...
        for (int i = 0; i < laserCloudSurfLastDSNum; i++)
        {
            ...
            if (pointSearchSqDis[4] < 1.0)
            {
                ...
                // 求maxA0中点构成的平面法向量
                // matB0是-1，这个函数用来求解AX=B的X，也就是AX+BY+CZ+1=0
     ...
     * 公式推导：https://blog.csdn.net/weixin_37835423/article/details/111587379
    ...
    void saveKeyFramesFullCloudToPcd()
    {
        static int i = 0;
        string saveKeyframeDirectory = std::getenv("HOME") + savePCDDirectory + "/keyFrameCloud/";
        pcl::PointCloud<PointType>::Ptr cloudOut(new pcl::PointCloud<PointType>());
        pcl::fromROSMsg(cloudInfo.cloud_deskewed, *cloudOut);
        pcl::io::savePCDFileBinary(saveKeyframeDirectory + "keyFrame_" + std::to_string(i++) + ".pcd", *cloudOut);
    }
    ...
    void addOdomFactor()
    {
        if (cloudKeyPoses3D->points.empty())
        {
            ...
            noiseModel::Diagonal::shared_ptr priorNoise = noiseModel::Diagonal::Variances((Vector(6) << 1e-2, 1e-2, 1e8, 1e-4, 1e-4, 1e-4).finished()); // rad*rad, meter*meter
    ...
    void gpsHandler(const nav_msgs::Odometry::ConstPtr &gpsMsg)
    {
        ...
        if (count < 10)
        {
            printf("--GPS0: lon: %.7f, lat: %.7f, alt: %.3f \n", gpsMsg->pose.pose.position.x, gpsMsg->pose.pose.position.y, gpsMsg->pose.pose.position.z);
            return;
        }
